...
### ▶ [Live demo](https://cloudscale-fleetops.vercel.app) · [API](https://fleetops-api-q9bg.onrender.com/swagger)

Sign in as `admin` / `fleetops-demo-2026`. Also try `manager` and `analyst` — the three roles
see different dashboards, and only the first two can change a vessel's status.

The API runs on a free instance that sleeps when idle, so the **first sign-in can take up to a
minute** while it wakes. Telemetry then updates every 20 seconds over SignalR with no page
refresh. Vessels and readings are generated by a simulator; see
[what is real and what is not](docs/adr/0006-live-ais-ingestion.md).
